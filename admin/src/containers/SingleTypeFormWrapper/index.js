...
const SingleTypeFormWrapper = ({ allLayoutData, children, slug }) => {
    ...
    const fetchData = async signal => {
      ...
      try {
        ...
        if (responseStatus === 403) {
          siapi.notification.info(getTrad('permissions.not-allowed.update'));
  ...
  const displayErrors = useCallback(err => {
    ...
    if (typeof errorMessage === 'string') {
      siapi.notification.error(errorMessage);
    }
  }, []);
  ...
  const onDelete = useCallback(
    async trackerProperty => {
      try {
        ...
        siapi.notification.success(getTrad('success.record.delete'));
  ...
  const onUnpublish = useCallback(async () => {
    ...
    try {
      ...
      siapi.notification.success(getTrad('success.record.unpublish'));
