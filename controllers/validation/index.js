...
const handleError = error => {
  throw siapi.errors.badRequest('ValidationError', formatYupErrors(error));
};
...
const validateUIDField = (contentTypeUID, field) => {
  const model = siapi.contentTypes[contentTypeUID];

  if (!model) {
    throw siapi.errors.badRequest('ValidationError', ['ContentType not found']);
  }
...
const validatePagination = ({ page, pageSize }) => {
  ...
  if (isNaN(pageNumber) || pageNumber < 1) {
    throw siapi.errors.badRequest('invalid pageNumber param');
  }
  if (isNaN(pageSizeNumber) || pageSizeNumber < 1) {
    throw siapi.errors.badRequest('invalid pageSize param');
  }
};
