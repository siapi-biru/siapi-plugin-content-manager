...
const { MANY_RELATIONS } = require('siapi-utils').relations.constants;
const { setCreatorFields } = require('siapi-utils');
...
module.exports = {
  ...
  async create(ctx) {
    ...
    await wrapBadRequest(async () => {
      ...
      await siapi.telemetry.send('didCreateFirstContentTypeEntry', { model });
    })();
  },
  ...
  async previewManyRelations(ctx) {
    ...
    if (assoc.nature === 'manyWay') {
      ...
      const assocModel = siapi.db.getModelByAssoc(assoc);
      relationList = await entityManager.findPage(
